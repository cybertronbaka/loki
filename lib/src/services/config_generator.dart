...
var _vIsRequired = ValidationRules(
  check: _isRequired,
  message: ([_]) => 'is required'
);
var _vIsString = ValidationRules(
  check: _isString,
  message: ([_]) => 'should be string'
);

var _vIsNullOrString = ValidationRules(
  check: _isNullOrString,
  message: ([_]) => 'should be string'
);

var _vIsNullOrYamlList = ValidationRules(
  check: _isNullOrYamlList,
  message: ([_]) => 'should be a list'
);

var _vIsNullOrListOfString = ValidationRules(
  check: (data) => data == null || (data.runtimeType == YamlList && (data as YamlList).every((e) => e.runtimeType == String)),
  message: ([_]) => 'should be a list of strings'
);

var _vIsNullOrYamlMap = ValidationRules(
  check: _isNullOrYamlMap,
  message: ([_]) => 'should be a map'
);

var _vIsYamlMap = ValidationRules(
  check: _isYamlMap,
  message: ([_]) => 'should be a map'
);

var _vIsNullOrBool = ValidationRules(
  check: _isNullOrBool,
  message: ([_]) => 'should be bool'
);
...
_ScriptConfigValidationRules _scriptsConfigRules = _ScriptConfigValidationRules(
  root: Validator(
    messagePrefix: ([command]) => chalk.yellowBright('scripts:$command (value)'),
    rules: [_vIsRequired, _vIsYamlMap]
  ),
  name: Validator(
    messagePrefix: ([command]) => chalk.yellowBright('scripts:$command:name'),
    rules: [_vIsNullOrString]
  ),
  description: Validator(
    messagePrefix: ([command]) => chalk.yellowBright('scripts:$command:description'),
    rules: [_vIsNullOrString]
  ),
  exec: Validator(
    messagePrefix: ([command]) => chalk.yellowBright('scripts:$command:exec'),
    rules: [_vIsRequired, _vIsString]
  ),
  command: Validator(
  ...
  workingDir: Validator(
    messagePrefix: ([command]) => chalk.yellowBright('scripts:$command:working_dir'),
    rules: [_vIsNullOrString]
  )
);
